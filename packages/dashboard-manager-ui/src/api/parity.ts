...
const RAW_ENTRIES: RawParityEntry[] = [
  // ── axi.yaml ──
  {
    ...
    features: [
      ...
      { id: 'axi-04', name: 'Pre-computed aggregates', status: 'partial', lev_equivalent: 'plugins/core-reactive/', action: 'adopt', notes: 'Combined operations to reduce round trips. Lev has reactive composition but not at CLI output level.' },
      ...
      { id: 'axi-07', name: 'Ambient context via session hooks', status: 'implemented', lev_equivalent: 'plugins/core-reactive/', action: 'implemented', notes: 'Reactive hooks system provides session lifecycle hooks. Direct parallel to AXI principle.' },
      ...
      { id: 'axi-09', name: 'Contextual disclosure (next-step templates)', status: 'partial', lev_equivalent: 'plugins/core-sdlc/', action: 'adopt', notes: 'Validation gates provide next-step guidance. Not yet as CLI command templates after every response.' },
  ...
  // ── dev-browser.yaml ──
  {
    ...
    measured_at: '2026-04-10',
    priority: 'P1',
    owner: 'core/cli + plugins/browser-cascade',
    metrics: [
    ...
    features: [
      { id: 'devb-01', name: 'Self-documenting CLI (--help IS the LLM guide)', status: 'partial', lev_equivalent: 'plugins/core-sdlc/', action: 'adopt', notes: 'dev-browser needs no separate SKILL.md. Lev has flowmind -> skill compiler but not yet self-documenting runtime.' },
  ...
  // ── ouroboros.yaml ──
  {
    ...
    measured_at: '2026-04-10',
    priority: 'P0',
    owner: 'plugins/core-sdlc + core/orchestration',
    metrics: [
    ...
    features: [
      { id: 'ooo-01', name: 'Socratic interview with ambiguity scoring', status: 'partial', lev_equivalent: 'plugins/core-sdlc/ (plan phase)', action: 'adopt', notes: 'Lev has planning flows but no Socratic interview with mathematical ambiguity gate. DNA constraints are the equivalent but applied to code, not requirements.' },
      ...
      { id: 'ooo-03', name: 'Double Diamond decomposition (Discover>Define>Design>Deliver)', status: 'partial', lev_equivalent: 'plugins/core-sdlc/flows/', action: 'adopt', notes: 'FlowMind flows implement this pattern. stabilization-loop.flow.yaml, plan-execute-verify. Not branded as Double Diamond.' },
      ...
      { id: 'ooo-05', name: 'Evolutionary loop with ontology convergence', status: 'partial', lev_equivalent: 'plugins/core-sdlc/flows/eval-harness.flow.yaml', action: 'adopt', notes: 'eval-harness produce->judge->compare->keep_or_discard is the same pattern. HOLD state = collect more samples. But no ontology similarity metric.' },
      ...
      { id: 'ooo-08', name: 'Ralph persistent loop across sessions', status: 'implemented', lev_equivalent: 'plugins/core-sdlc/flows/lev-ralph.flow.yaml', action: 'implemented', notes: 'lev-ralph IS this. Same name even. Work->gate->review->done. Fresh context per iteration.' },
  ...
  // ── hermes.yaml ──
  {
    ...
    measured_at: '2026-04-10',
    priority: 'P1',
    owner: 'plugins/core-platforms + core/memory',
    lineage: [
  ...
  // ── omx.yaml ──
  {
    ...
    measured_at: '2026-04-10',
    priority: 'P0',
    owner: '.worktrees/lev-omx + plugins/core-sdlc',
    lineage: [
    ...
    features: [
      { id: 'omx-01', name: '4-lane operator model (deep-interview > ralplan > team/ralph > verify)', status: 'partial', lev_equivalent: 'plugins/core-sdlc/flows/', action: 'adopt', notes: 'The 4-lane model IS the day-to-day UX. Lev has the flows but not the unified surface.' },
      ...
      { id: 'omx-04', name: 'Ralph persistent loop with architect verification', status: 'implemented', lev_equivalent: 'plugins/core-sdlc/flows/lev-ralph.flow.yaml', action: 'implemented', notes: 'Same pattern, same name. lev-ralph is the canonical implementation.' },
  ...
  // ── omo.yaml ──
  {
    ...
    metrics: [],
    features: [
      { id: 'omo-01', name: 'Category routing (task classification > mode selection)', status: 'partial', lev_equivalent: 'plugins/core-sdlc/', action: 'extract', notes: 'OMO routes tasks to categories before execution. Lev has flow selection but not auto-classification.' },
      { id: 'omo-02', name: 'Planner/executor split', status: 'implemented', lev_equivalent: 'plugins/core-sdlc/flows/ (plan-execute-verify)', action: 'implemented', notes: 'Lev already has this pattern in multiple flows.' },
