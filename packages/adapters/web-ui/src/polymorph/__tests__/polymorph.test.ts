...
describe('renderToHTML', () => {
  ...
  it('includes theme colors in the output', () => {
    ...
    // Skynet bg
    expect(html).toContain(THEMES.skynet.bg);
  });
...
describe('renderToPencil', () => {
  it('returns an array of operation strings', () => {
    ...
    const ops = renderToPencil(primitives, '"root"', { theme: 'terminal-swiss' });
  ...
  it('each operation starts with a binding assignment', () => {
    ...
    const ops = renderToPencil(primitives, '"parent"', { theme: 'skynet' });
    for (const op of ops) {
...
// ---------------------------------------------------------------------------
describe('THEMES', () => {
  const themeNames = ['terminal-swiss', 'skynet', 'system'] as const;
  ...
  for (const name of themeNames) {
    ...
    for (const key of requiredKeys) {
      it(`"${name}" has token "${key}"`, () => {
        expect(THEMES[name][key]).toBeTruthy();
        expect(typeof THEMES[name][key]).toBe('string');
      });
    }
  }
});
