...
function variantColor(p, theme, themeMode) {
    const t = getThemeTokens(theme, themeMode);
    const v = p.props.variant ?? 'default';
    const map = {
 ...
 */
export function renderToPencil(primitives, parentBinding, options) {
    const ops = [];
    const t = getThemeTokens(options.theme, options.themeMode);
    for (const p of primitives) {
        ...
        switch (p.kind) {
            case 'status-dot': {
                ...
                const color = statusColors[p.props.status ?? 'offline'] ?? t.muted;
            ...
            case 'badge': {
                const bg = variantColor(p, options.theme, options.themeMode);
            ...
            case 'button': {
                const bg = (p.props.variant === 'primary' || p.props.variant === 'danger') ? variantColor(p, options.theme, options.themeMode) : 'transparent';
            ...
            case 'text-block': {
                ...
                ops.push(`${binding}=I(${parentBinding}, {"type": "text", "name": "Text", "content": "${(p.props.content ?? '').replace(/"/g, '\\"')}", "fontSize": ${fontSize}, ${weight} "textColor": "${color}"})`);
                break;
            }
            case 'input-field': {
                ...
                ops.push(`${binding}_placeholder=I(${binding}_input, {"type": "text", "content": "${p.props.placeholder ?? ''}", "fontSize": ${sz.fontSize}, "textColor": "${t.muted}"})`);
                break;
            }
            case 'progress-bar': {
                const pct = Math.min(100, Math.max(0, (p.props.value ?? 0) / (p.props.max ?? 100) * 100));
                const barColor = variantColor(p, options.theme, options.themeMode) === t.muted
                    ? t.accent
                    : variantColor(p, options.theme, options.themeMode);
            ...
            case 'metric-value': {
                ...
                ops.push(`${binding}_value=I(${binding}, {"type": "text", "content": "${String(p.props.value ?? '')}${p.props.unit ?? ''}", "fontSize": ${sz.fontSize + 12}, "fontWeight": "700", "textColor": "${t.text}"})`);
