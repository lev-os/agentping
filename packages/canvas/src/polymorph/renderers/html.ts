 ...
 * HTML Renderer — v2
 *
 * @deprecated Retained for export-only artifact generation. Not a production surface.
 * Use `renderToReact` (interactive) or `renderToPencil` (design) instead.
 *
...
function renderPrimitive(p: PolymorphPrimitive, t: ThemeTokens): string {
  ...
  switch (p.kind) {
    ...
    case 'check-item': {
      ...
      const disabled = p.props.disabled ? 'disabled' : '';
      const checkedClass = p.props.checked ? 'ap-check--checked' : '';
      return `<label class="${cls('ap-check', checkedClass, Boolean(p.props.disabled) && 'ap-check--disabled')}" data-id="${id}">
...
<body>
  <div class="ap-shell">
    <div class="ap-header">
      ...
      <div class="ap-header__meta">${esc(options.template)} &middot; ${esc(options.theme)} &middot; ${esc(options.themeMode)}</div>
    </div>
    <div class="ap-grid">
      ${body}
    </div>
