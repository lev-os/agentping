...
function variantColor(p: PolymorphPrimitive, theme: ThemeName, themeMode: ThemeMode): string {
...
export function renderToPencil(
  ...
  options: { theme: ThemeName; themeMode: ThemeMode },
): string[] {
  const ops: string[] = [];
  ...
  for (const p of primitives) {
    ...
    switch (p.kind) {
      ...
      case 'progress-bar': {
        ...
        const barColor =
          variantColor(p, options.theme, options.themeMode) === t.muted
            ? t.accent
