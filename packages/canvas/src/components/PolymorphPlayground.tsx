  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {typeof p.props.secondary === 'string' && (
...
function NavItemView({ p, t, emit }: PrimProps) {
  ...
  return (
    <div
      ...
      role="button" tabIndex={0}
    >
      {typeof p.props.icon === 'string' && <span style={{ fontSize: 14 }}>{p.props.icon}</span>}
      <span style={{ flex: 1 }}>{p.label}</span>
      {typeof p.props.badge === 'string' && (
        <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 10, background: `${t.accent}26`, color: t.accent, fontWeight: 500 }}>
          {p.props.badge}
        </span>
      )}
    </div>
...
function CardView({ p, t, emit }: PrimProps) {
  return (
    <div style={{
      ...
      {typeof p.props.title === 'string' && (
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{p.props.title}</div>
      )}
      {typeof p.props.subtitle === 'string' && (
        <div style={{ fontSize: 12, color: t.muted, marginBottom: 12 }}>{p.props.subtitle}</div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(p.children ?? []).map((child) => <PrimitiveView key={child.id} p={child} t={t} emit={emit} />)}
      </div>
    </div>
  );
}
...
export function PolymorphPlayground() {
  const [activeTemplate, setActiveTemplate] = useState('design');
  ...
  const theme = getThemeTokens(activeTheme, activeThemeMode);

  resetIds();
  const template = templates[activeTemplate];
  const primitives = template?.previewRenderer({ topic: `${activeTemplate} Explorer` }) ?? [];
          ...
          {(['dark', 'light'] as ThemeMode[]).map((mode) => (
            <button
              ...
              style={{
                ...
                fontSize: 11,
                borderRadius: 6,
                border: `1px solid ${activeThemeMode === mode ? theme.accent : theme.border}`,
                cursor: 'pointer',
                fontFamily: 'inherit',
                background: activeThemeMode === mode ? `${theme.accent}1a` : 'transparent',
                color: activeThemeMode === mode ? theme.accent : theme.muted,
                transition: 'all 150ms',
              }}
            >
              {mode}
