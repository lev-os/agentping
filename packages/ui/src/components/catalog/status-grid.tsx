  ...
  return (
    <div className={`status-grid-container ${className}`}>
      <div className={`status-grid columns-${columns}`}>
        {cards.map((card) => {
          const icon = card.icon || DEFAULT_ICONS[card.status];
          ...
          return (
            ...
            >
              <div className="status-card-header">
                <div className={`status-icon ${card.status}`}>{icon}</div>
                <div className={`status-indicator ${card.status}`}>{statusLabel}</div>
              </div>
