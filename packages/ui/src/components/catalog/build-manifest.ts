...
const OUTPUT_FILE = join(CATALOG_DIR, "_manifest.json");

function scan(): { entries: ManifestEntry[]; missing: string[]; invalid: string[] } {
  const files = readdirSync(CATALOG_DIR).filter((f) => f.endsWith(".tsx"));
  ...
  for (const tsx of files) {
    ...
    const manifestPath = join(CATALOG_DIR, `${id}.manifest.json`);
